            ...
            <div className="card-text px-4 py-2">
              ...
              <div className="flex justify-between mb-3">
                ...
                <p className="text-sm text-gray-600 flex items-center">
                  ...
                  <span>
                    {
                      resp.data.reviews[0].rate <= 1 ? '"Very Poor"' : 
                      resp.data.reviews[0].rate > 1 && resp.data.reviews[0].rate <= 2 ? '"Poor"' :
                      resp.data.reviews[0].rate > 2 && resp.data.reviews[0].rate <= 3 ? '"Fair"' :
                      resp.data.reviews[0].rate > 3 && resp.data.reviews[0].rate <= 4 ? '"Very Good"' :
                      '"Excellent"'
                    }
                  </span>
                </p>
              ...
              <div className="details flex justify-between flex-wrap mb-3">
                ...
                <p className="text-sm text-gray-600 w-1/2 mb-3 flex items-center">
                  ...
                  <span>{resp.data.aboutRental.area}m<sup>2</sup></span>
                </p>
                <p className="text-sm text-gray-600 w-1/2 mb-3 flex items-center" style={{display: resp.data.features.airConditioner == false ? "none" : "true"}}>
                  <FaFan className="mr-3 text-lg"/> A/C
                </p>
              </div>
              <Link to={`/rental-details/${resp.id}`}>
                <button type="button" className="property-button text-white bg-cyan-600 hover:bg-cyan-800 font-medium rounded-lg text-sm px-8 py-2.5 mr-2 mb-2 dark:bg-cyan-600 dark:hover:bg-cyan-800 focus:outline-none dark:focus:ring-cyan-800">Property Details</button>
              </Link>
            </div>
          </Card>
        </div>
